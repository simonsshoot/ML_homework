...
训练前仔细检查自己的格式是否满足要求,该库要求数据集格式为VOC格式,需要准备好的内容有输入图片和标签
...
if __name__ == "__main__":
    ...
    #---------------------------------------------------------------------#
        #ps:如果是在faster-rcnn-pytorch--master下运行Python train.py，这里model_path就不应写成：./faster-rcnn-pytorch-master/model_data/voc_classes.txt
    #这样会导致多嵌套一层faster-rcnn-pytorch-master，导致找不到voc_classes
    classes_path    = './model_data/new_elec_classes.txt'
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    model_path      = './pretrained_models/voc_weights_resnet.pth'
    #------------------------------------------------------#
    ...
    '''
    annotation存xml格式的标签
    对于2007_train.txt的数据，举个例子说明：
    /home/yx/yx_search/search/new-rcnn/Faster-RCNN-Pytorch/faster-rcnn-pytorch-master/VOCdevkit/VOC2007/JPEGImages/000005.jpg 263,211,324,339,8 165,264,253,372,8 241,194,295,299,8
    有三个框，第一个框的坐标为(263,211,324,339)，第二个框的坐标为(165,264,253,372)，第三个框的坐标为(241,194,295,299)，8的含义是类别

    '''
    # train_annotation_path   = '2025_train.txt'
    # val_annotation_path     = '2025_val.txt'
    train_annotation_path   = '2025_train_detail.txt'
    val_annotation_path     = '2025_val_detail.txt'
    ...
    wanted_step = 5e4 if optimizer_type == "sgd" else 1.5e3
    ...
    if True:
        ...
        # ------------------------------------#
        #   冻结bn层
        #冻结 BN 层可以保持主干网络的归一化效果，避免因参数更新而导致的特征分布变化。
        # ------------------------------------#
        model.freeze_bn()
...
仅更新 RPN 的权重，生成初步候选框。
​​训练 Fast R-CNN
使用 RPN 生成的候选框训练 Fast R-CNN，不更新共享主干的权重。
​​再次训练 RPN
用 Fast R-CNN 更新后的权重初始化 RPN，但冻结共享主干，仅微调 RPN 独有的层。
​​微调 Fast R-CNN
保持共享主干的权重不变，仅更新 Fast R-CNN 的检测头。

这里的代码似乎是通过​冻结主干+分阶段训练​​达到类似效果
冻结阶段（Freeze_Epoch=50）
仅训练 RPN 和检测头，主干网络权重固定。
目的：快速收敛检测头，避免破坏主干特征。
解冻阶段（UnFreeze_Epoch=100）
解冻主干网络，微调全部参数。
目的：进一步优化特征提取和检测头的协同性能。
'''